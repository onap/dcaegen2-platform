...
import com.att.policycreate.exception.PolicyCreateException;
import com.att.policycreate.model.PolicyModel;
    ...
    public void createPolicyModels(Parameters[] params, String filePath) {
        try {
            ...
            for (String s : policyGroups) {
                ...
                if (!response.get("hasEntrySchema").equals(""))
        ...
        for (Parameters p : params) {
            if (p.isPolicy_editable()) {
                if (names.isEmpty()) {
                    ...
                    if (!names.contains(p.getPolicy_group())) {
                        names.add(p.getPolicy_group());
                    }
                }
            }
