...
cup_without_current_pid=$(echo "$cup_same_process" | grep -v $process)
process_count=$(echo $cup_without_current_pid | grep -c "$temp_process")
if [ $process_count -ge 1 ] 
then
    dcaelogdebug "%s %s %s %s" "WARN" "There is another instance of the process" "$process" "is running. Quitting startAAIRealtimeUpdatesForDB.sh Process"
    echo $0 INFO "There is another instance of the process $prog is running. Quitting startAAIRealtimeUpdatesForDB.sh Process"
    exit 0
fi
...
$JAVA_HOME/bin/java -Dlogback.configurationFile=$DTI/config/logger/logback.xml -Dhttps.protocols=TLSv1.2 com.att.vcc.inventorycollector.InventoryCollector UPDATES YES
if [ $? == 0 ]; then
