CREATE OR REPLACE VIEW dti.rt_generic_vnf_vserver_v AS
SELECT
g.VNF_ID,
g.VNF_NAME,
g.VNF_NAME2,
...
ON g.vnf_id = r.from_node_id
AND r.related_FROM = 'generic-vnf'
INNER JOIN dti.rt_vserver v
ON CONCAT(v.cloud_owner, '|', v.cloud_Region_id, '|', v.tenant_id, '|', v.vserver_id) = r.to_node_id
AND r.related_to = 'vserver'
