...
LOGFILE=$LOGDIR/VCC_startFileMonitor_$today.log
mkdir -p $LOG_DIR 2>/dev/null

if [[ ! $DTI_DATA_DIR ]]; then          # DTI_DATA_DIR is NOT set, then it is NOT in k8s, exit
        echo "$now|DTI_DATA_DIR is NOT set, it is NOT in k8s, do not start FileMonitor" >> $LOGFILE
        exit
fi
...
if test -n "$pid"
then
        echo "$now|startFileMonitor::FileMonitor is already running, pid=$pid" >> $LOGFILE
else
...
java $LOG4J_OPTIONS -DVCC_HOME=${VCC_HOME} -cp $CLASSPATH -Dcurrent_machine=$localhost com.att.vcc.configmanager.watcher.FileMonitor $dir & >> $LOGFILE 2>&1 &
        sleep 2
        pid=`ps -aefu $LOGNAME | grep "com.att.vcc.configmanager.watcher.FileMonitor" | awk '{if ($8!="grep") print $2}'`
        echo "$now|startFileMonitor::Started FileMonitor pid=$pid, directory=$dir" >> $LOGFILE
fi
