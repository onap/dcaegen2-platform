...
python -m dtihandler 2>&1 &
PID=$!

function finish {
  echo "killing dti_handler ${PID}" $(date +%Y_%m%d-%H:%M:%S.%N)
  kill -9 ${PID}
  echo "killed dti_handler ${PID}" $(date +%Y_%m%d-%H:%M:%S.%N)
}
trap finish SIGHUP SIGINT SIGTERM

echo "running dti_handler as" ${PID} "log" ${LOG_FILE}
