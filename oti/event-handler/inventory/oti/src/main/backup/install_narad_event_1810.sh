...
then
        print "keystore directory does not exist. so creating it ..."
        mkdir -p ${VCC_HOME}/keystore
        print "copy certificate files from /opt/app/dcae-certificate to keystore directory ..."
...
then
        print "policy directory does not exist. so creating it ..."
        mkdir -p ${VCC_HOME}/config/policy
fi

echo "FINISHED install.sh for 1810"
