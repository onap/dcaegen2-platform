...
then
    dcaelogdebug "%s %s %s %s" "WARN" "There is another instance of the process" "$process" "is running. Quitting getNARADRealTimeEventsDB.sh Process"
    echo $0 INFO "There is another instance of the process $prog is running. Quitting getNARADRealTimeEventsDB.sh Process"
    exit 0
fi
...
$JAVA_HOME/bin/java -Dlogback.configurationFile=$DTI/config/logger/logback_naraddb.xml -Dhttps.protocols=TLSv1.2 com.att.vcc.inventorycollector.InventoryCollector NARAD yes
if [ $? == 0 ]; then
        STATUSCODE=COMPLETE RESPCODE=200 RESPDESC="Operation completed successfully" TARGETENTITY="NARAD DB" TARGETSVCNAME="$java_program" TARGETVENTITY=$REMOTESERVER \
        dcaelogmetrics "The child script %s completed successfully" "$java_program"
else
        STATUSCODE=FAILURE RESPCODE=101 RESPDESC="Operation failed" TARGETENTITY="NARAD DB" TARGETSVCNAME="$java_program" TARGETVENTITY=$REMOTESERVER \
