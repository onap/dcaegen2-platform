...
class CBSRest(object):
    _logger = logging.getLogger("dti_handler.cbs_rest")
    ...
    def get_service_component_all(service_name, service_location=None, policies_as_list=False):
        ...
        if r_dict and r_dict.get('dti'):
            r_dict['dti'] = CBSRest.get_dti(service_name, service_location=service_location)
        return r_dict

    @staticmethod
    def get_dti(service_name=None, vnf_type=None, vnf_id=None, service_location=None):
        """
        Get DTI events.
        ...
        if lc_vnf_type and ',' not in lc_vnf_type:
            ...
            if vnf_id:
                # get just one vnf_id
                t_dict = CBSRest.get_value("dti_events/" + type + "/" + vnf_id, default=None)
                if t_dict:
            ...
            # ... and not vnf_id
                ...
                t_dict = CBSRest.get_kvs("dti_events/" + type + "/")
                if t_dict:
                    if not want_locs:
                        r_dict = copy.deepcopy(t_dict)
                    else:
        ...
        # If they did not specify either service_name or vnf_type (the only way give_types=[])
            ...
            t_dict = CBSRest.get_kvs("dti_events/")
            if t_dict:
                for type in t_dict:
