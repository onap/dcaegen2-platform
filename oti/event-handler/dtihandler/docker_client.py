...
from dtihandler.consul_client import ConsulClient, ConsulClientKVEntryNotFoundError, ConsulClientServiceNotFoundError, ConsulClientNodeNotFoundError
from dtihandler.utils import decrypt
...
class DockerClient(object):
    ...
    _logger = logging.getLogger("dti_handler.docker_client")
    ...
    def ndtify_for_reconfiguration(self, container_id, cmd):
        """Ndtify Docker container that reconfiguration occurred

        Ndtify the Docker container by doing Docker exec of passed-in command
        ...
        for attempts_remaining in range(11,-1,-1):
            try:
                ...
                if e.explanation and 'is restarting' in e.explanation.lower():
                    DockerClient._logger.debug("ndtification exec_create() experienced: {!s}".format(e))
                ...
                # elif e.explanation and 'is not running' in e.explanation.lower():
                    ...
                    DockerClient._logger.warn("aborting ndtification exec_create() because exception {}: {!s}".format(type(e).__name__, e))
                    return str(e)  # don't raise or CM will retry usually forever
                    # raise DockerClientError(e)
            except Exception as e:
                DockerClient._logger.warn("aborting ndtification exec_create() because exception {}: {!s}".format(
                    type(e).__name__, e))
                return str(e)  # don't raise or CM will retry usually forever
                # raise DockerClientError(e)
            else:
                break
        if not result:
            DockerClient._logger.warn("aborting ndtification exec_create() because docker exec failed")
            return "ndtification unsuccessful"  # failed to get an exec_id, perhaps trying multiple times, so don't raise or CM will retry usually forever
        DockerClient._logger.debug("ndtification exec_create() succeeded")

        for attempts_remaining in range(11,-1,-1):
            try:
                result = self._client.exec_start(exec_id=result['Id'])
            except Exception as e:
                DockerClient._logger.debug("ndtification exec_start() got exception {}: {!s}".format(type(e).__name__, e))
                if attempts_remaining == 0:
                    DockerClient._logger.warn("aborting ndtification exec_start() because exception {}: {!s}".format(type(e).__name__, e))
                    return str(e)  # don't raise or CM will retry usually forever
                    # raise DockerClientError(e)
                time.sleep(10)
            else:
                break
        DockerClient._logger.debug("ndtification exec_start() succeeded")
