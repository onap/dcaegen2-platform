...
#uncomment the below line if the vpe connectivity is available.
RSYNC_OPTS="-aqz --chmod=g+w"
#comment the below line if the vpe connectivity is available.
#RSYNC_OPTS="-aqz --exclude configs_vpe.txt --chmod=g+w"
...
BASEDIR=/spool/gamma/vec/$HOSTNAME/
READY_FOR_PROC=/spool/gamma/vec/$HOSTNAME/
VEC_HOME=/opt/app/vec
REMOTEDIR=${VEC_HOME}/data/input/VEC/
LOGFILE=/home/netman/log/vecTopo-transfer
ARCHIVEDIR=/spool/gamma/archive/
##############################################################
...
do
    ...
    then
        ...
        if [ $? == 0 ]; then
			STATUSCODE=COMPLETE RESPCODE=200 RESPDESC="Operation completed successfully" TARGETENTITY="rsync" TARGETSVCNAME="rsync" TARGETVENTITY=${SERVER} \
			dcaelogmetrics "The child script %s completed successfully" "rsync"
		else
			STATUSCODE=FAILURE RESPCODE=101 RESPDESC="Operation failed" TARGETENTITY="rsync" TARGETSVCNAME="rsync" TARGETVENTITY=${SERVER} \
			dcaelogmetrics "The child script %s failed" "rsync"
			ERRORCATEGORY=ERROR ERRORCODE=102 RESPDESC="child script failed" \
			dcaelogerror "%s %s" "ERROR" "rsync failed"
		fi
        dcaelogdebug "DEBUG" "server=$HOSTNAME,feed=${k}:transfer complete"
        fileSent=$NUM_FILES
        if [ ! -d $ARCHIVEDIR${k}/$HOSTNAME ]; then
            mkdir -p $ARCHIVEDIR${k}/$HOSTNAME
        fi
        timestamp=$(date +%Y%m%d%H%M%S)
        for f in $(ls configs_*.txt)
        do
            echo "file is $f"
