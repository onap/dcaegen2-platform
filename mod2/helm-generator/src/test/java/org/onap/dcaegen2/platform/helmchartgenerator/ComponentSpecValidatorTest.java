...
public class ComponentSpecValidatorTest {
    ...
    void validateTemplateStructure() {
        String specSchema = "src/test/input/specs/schemas/specSchema.json";
        assertValidSpec(specSchema);
        assertInvalidSpec(specSchema);
    }

    private void assertValidSpec(String specSchema) {
    ...
    private void assertInvalidSpec(String specSchema) {
        assertInvalidSchema(specSchema);
        assertNoHelmThrowsError(specSchema);
        assertNoServiceThrowsError(specSchema);
    }

    private void assertInvalidSchema(String specSchema) {
        String specFileLocation = "src/test/input/specs/invalidSpecSchema.json";
        assertThrows(RuntimeException.class, () -> validator.validateSpecFile(specFileLocation, specSchema));
    }

    private void assertNoHelmThrowsError(String specSchema) {
        String specFileLocation = "src/test/input/specs/invalidSpecNoHelm.json";
        assertThrows(RuntimeException.class, () -> validator.validateSpecFile(specFileLocation, specSchema));
    }

    private void assertNoServiceThrowsError(String specSchema) {
