...
/*  Commented for future use to handle the
    @Test
    void test_createPolicyModel_shouldReturn201AndResponseBody() throws Exception {
        PolicyModelCreateRequest policyModelCreateRequest = getPolicyModelCreateRequest();
        PolicyModel policyModel = getPolicyModelResponse();

        when(mockPolicyModelService.createPolicyModel(policyModelCreateRequest, USER)).thenReturn(policyModel);

        mockMvc.perform(post("/api/policy-model/").param("user", USER)
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(policyModelCreateRequest)).accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated());
        verify(mockPolicyModelService, times(1)).createPolicyModel(policyModelCreateRequest,USER);
    }*/
